'use strict';
angular.module('ciscoExecDashApp').controller('CollaborationRefreshController', ['$scope', '$rootScope', '$http', 'RestUri', '$sessionStorage', 'FiltersServ', 'CollaborationRefreshServ', '$filter', '$timeout', '$uibModal', 'CiscoUtilities', function ($scope, $rootScope, $http, restUri, $sessionStorage, filtersServ, CollaborationRefreshServ, $filter, $timeout, $uibModal, CiscoUtilities) {

    $rootScope.analysis = 'campaign';
    $rootScope.dashboard = 'collaborationRefresh';
    ...
    filtersServ.showInsightFilters('collaborationRefresh');
    ...
    //Reloading data on applying any filters
    ...
    $scope.getData = function(){
        ...
        CollaborationRefreshServ.getData(filtersToBeApplied).then(function (res) {
            ...
            $timeout(function () {
                getCollaborationRefreshGridHeight();
            }, 500);
        });
    };
    ...
    $scope.gridOptions = {
        ...
        onRowGroupOpened: function (params) {
            ...
            if (params.node.expanded && !params.data.customersLoaded) {
                ...
                CollaborationRefreshServ.getCustomers(filters).then(function (res) {
            ...
            if (params.data.addCustomers) {
                $http.get('data/campaigns/cr_customers_more.json')
                    .then(function (res) {
    ...
    $scope.onBtExport = function () {
        ...
        var params = {
            columnGroups: true,
            fileName: "Collaboration_Refresh"
        }
   ...
   $scope.sliderTextColor = function (key, value, d) {
    ...
    $scope.openCollaborationDefinition = function () {
            $uibModal.open({
                templateUrl: 'views/modal/collaboration-refresh-definition.html',
 ...
 $scope.allowGenerateCollab = function (c) {
    ...
    $scope.getadvancedFilterObject = function(){
      ...
      var collabRportEvent = $scope.$on('create-collab-proposal', function(event, collabData, savName){
            $scope.collabData = angular.copy(collabData);
            $scope.openRequestReport(savName,"Create Collab Presentation and Proposal");
        });
    ...
    $scope.openRequestReport = function(reportType) {
            ...
            activeReportTab = "collaborationRefresh";
            var nodeName = $scope.gridOptions.api.getSelectedRows();
            var savId = [];
            var savName ="";
            var salesLevel = "";
            var collaborationData = $scope.collabData;
            ...
            if (reportType === "Request Customer Detail Report"  || reportType === "Request Smart IB Report" || reportType === "Create Collab Presentation and Proposal") {
                var multipleState = [];
                var multipleStateId = [];
                ...
                for(var a=0; a<nodeName.length; a++){
                    ...
                    multipleStateId.push(nodeName[a].savID);
                     multipleSalesLevel.push(nodeName[a].salesLevel);
                }
                salesLevel = multipleSalesLevel;
                nodeName = multipleState; // Change for DE163261
                savId = multipleStateId;
            }else{
                savId = nodeName[0].savID;
                salesLevel = nodeName[0].salesLevel;
                nodeName = nodeName[0].savName; // Change for DE163261
            ...
            CollaborationRefreshServ.getSmartIbReport(nodeName, index, activeReportTab, $scope.lineCountSelected, $scope.listamountselected, filterObj, savId,collaborationData,salesLevel).then(function() {
                if(index === 0){
                    $uibModal.open({
