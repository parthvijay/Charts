...
var drsTable = function () {
    setTimeout(function () {
        $('.drs-table > tbody').unbind('scroll').scroll(function () {
            var shadowValue = $(".drs-table").closest('div.kpi').hasClass("expanded") ? 3 : 2;
...
var d3_wrap = function (text, width) {
...
var downloadCSV = function (x) {
    ...
    if (YOYColumnNum != -1) {
        $(negativeYOYColumns).each(function (index) {
            $(this).html(spanChildArray[index]);
        });
    }
    //DE131090
    if(kpi.find('table.route-to-market').length >= 1) {
        ...
        table.find('tbody > tr').each(function (a, b) {
            var sr = 1;
            var obj = {};
            obj[arr[0]] = $(b).find('.fix-long-word').text();
        ...
        var label_3 = isDrilled ? 'Product Family' : 'Product Family';
    ...
    JSONToCSVConvertor(json, title.trim(), true);
};
